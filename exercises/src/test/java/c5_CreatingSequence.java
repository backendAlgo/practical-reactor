...
//        generateFlux.log().subscribe(data -> System.out.println(data));
//        //------------------------------------------------------
...
//            Thread first = new Thread(() -> IntStream.range(0, 10).forEach(i -> sink.next(i)));
//            Thread second = new Thread(() -> IntStream.range(10, 20).forEach(i -> sink.next(i)));
//            first.start();
//            second.start();
//            try {
//                first.join();
...
//            }
//        });
//        createFlux.subscribe(data -> System.out.println(data));
//
//        //------------------------------------------------------
//
        class RestTemplate {
            public String get() {
                return "response from other service get";
            }
        }
        Flux<Integer> createFlux = Flux.create(sink -> {
            new Thread(() -> {
