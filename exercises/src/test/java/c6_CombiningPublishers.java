...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void behold_flatmap() {
        ...
        Mono<String> currentUserEmail = getCurrentUser().flatMap(userString -> {
            return getUserEmail(userString);
        });
    ...
    @Test
    public void flatvsconcatexample() throws InterruptedException {
        Flux<String> userNames = Flux.just("Jamol", "Kamol222", "Sob", "Bobur2222222", "Olamgir411241421241414");
        Flux<Integer> lengthUser = userNames.concatMap(name -> {
            if (name.equals("Sob")) {
                return Flux.never();
            } else {
                System.out.println(name);
                return Flux.just(name.length());
            }
        });
        lengthUser.subscribe(data -> System.out.println(data));
        Thread.sleep(5000);
    }
    ...
    public void mail_box_switcher() {
        ...
        Flux<Message> myMail = mailBoxPrimary().switchOnFirst((signal, flux) -> {
            if (signal.isOnNext() && signal.get().metaData.equals("spam")) {
    ...
    public void prettify() {
        ...
        Mono<Boolean> successful = openFile().then(
    ...
    public void acid_durability() {
        ...
        Flux<String> committedTasksIds = tasksToExecute()
                .concatMap(taskMono ->
                        taskMono.flatMap(taskId -> commitTask(taskId)
                                .thenReturn(taskId)));
